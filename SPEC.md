...
Wayland-Wheeltani turns **hold middle mouse button + vertical pointer motion**
into progressive continuous wheel scrolling on Linux Wayland desktops.
...
supports pointer motion, standard mouse buttons, legacy wheel detents, and
hi-res wheel units where 120 hi-res units equal one legacy detent.
...
If the middle button is released before vertical motion exceeds the configured
deadzone, the engine emits a synthetic middle click. This preserves common
desktop behavior such as opening links in new tabs or paste-primary-selection.

## Progressive speed

Vertical offset from the original press position controls scroll speed.

V1 uses configurable distance steps by default:

1. offset inside `deadzone_units` => no scroll;
2. each `scroll_speed_steps` entry maps an absolute distance from the original
   press point to a speed in wheel detents per second;
...
deadzone. Crossing the press point reverses direction.
